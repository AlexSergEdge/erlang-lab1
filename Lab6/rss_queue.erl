...
%% @doc функция упрощает отправку элемента в очередь
add_item(QPid,Item)->
  ok = gen_server:cast(QPid , {add_item,Item} ),ok.
%% @doc функция извлекает элементы из документа ленты, 
%% и отправляет по порядку в очередь
add_feed(QPid,RSS2Feed)->
 ...
 ok. 
%% @doc функция упрощает получение списка элементов ленты от процесса
get_all(QPid)->
  gen_server:call(QPid,{get_all}).
%% @doc передача объекта нескольким PID
